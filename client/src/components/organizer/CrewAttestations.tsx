...
import { FiUser, FiStar, FiBriefcase, FiCalendar, FiCheckCircle, FiAlertCircle } from 'react-icons/fi';
...
export const CrewAttestations = () => {
    ...
    <Card key={crew.id} bg="background.tertiary" overflow="hidden">
      <CardHeader pb={0}>
        <Flex align="center" gap={3}>
          ...
          <Box>
            <Heading size="sm" fontWeight="semibold">{crew.name}</Heading>
            <Text color="text.secondary" fontSize="sm">{crew.role}</Text>
          </Box>
          {crew.verified && (
            <Badge colorScheme="blue" ml="auto">Verified</Badge>
          )}
        </Flex>
      </CardHeader>
      <CardBody>
  ...
  return (
    <Box p={5}>
      <Heading size="lg" mb={5}>Crew Attestations</Heading>
      <Text mb={6} color="text.secondary">
        Attestations provide a verified record of your experience working with crew members. Completing attestations helps crew members build their reputation in the industry.
      </Text>

      <Tabs variant="soft-rounded" colorScheme="blue" mb={8}>
        <TabList mb={5}>
          <Tab>Pending ({pendingAttestations.length})</Tab>
          <Tab>Completed ({completedAttestations.length})</Tab>
          <Tab>Available ({availableToAttest.length})</Tab>
        </TabList>
        <TabPanels>
          <TabPanel p={0}>
            {pendingAttestations.length > 0 ? (
              ...
              <Box p={10} textAlign="center" bg="background.tertiary" borderRadius="md">
                ...
                <Heading size="md" mb={2}>No Pending Attestations</Heading>
                <Text color="text.secondary">You don't have any pending crew attestations at this time.</Text>
              </Box>
            )}
          </TabPanel>
          <TabPanel p={0}>
            {completedAttestations.length > 0 ? (
              ...
              <Box p={10} textAlign="center" bg="background.tertiary" borderRadius="md">
                ...
                <Heading size="md" mb={2}>No Completed Attestations</Heading>
          ...
          <TabPanel p={0}>
            {availableToAttest.length > 0 ? (
              ...
              <Box p={10} textAlign="center" bg="background.tertiary" borderRadius="md">
                <Icon as={FiAlertCircle} boxSize={10} color="text.secondary" mb={3} />
                <Heading size="md" mb={2}>No Available Crew Members</Heading>
                <Text color="text.secondary">There are no crew members available for attestation at this time.</Text>
              </Box>
            )}
          </TabPanel>
        </TabPanels>
...
export default CrewAttestations;
